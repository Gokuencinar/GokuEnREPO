# BandLock 0.6.30 Global App

BandLock 0.6.30 moves the public tweak out of the iOS Settings app and into a dedicated UIKit application installed on the Home Screen.

### 0.6.30 MobilePhone keypad bridge diagnostic
...
- A tiny tweak is filtered only to `com.apple.mobilephone`. BandLock opens Phone, sends a Darwin notification, and the bridge reproduces the keypad sequence inside MobilePhone so the same special-code path as manual entry is used.
...
BandLock 0.6.30 continues to target LTE/4G band control only; it does not claim 5G NR band locking.
