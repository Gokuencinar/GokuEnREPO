...
- (void)quickActionTapped:(UIButton *)sender {
    BLDiagLog([NSString stringWithFormat:@"band-picker quick action %ld", (long)sender.tag]);
    @try {
        ...
        BLDiagLog([NSString stringWithFormat:@"band-picker quick exception %@ %@", exception.name, exception.reason]);
    }
}
...
- (void)bandSwitchChanged:(UISwitch *)sender {
    ...
    BLDiagLog([NSString stringWithFormat:@"band switch section=%ld row=%ld on=%d", (long)section, (long)row, sender.isOn]);
    @try {
        ...
        BLDiagLog([NSString stringWithFormat:@"band switch exception %@ %@", exception.name, exception.reason]);
    }
}
