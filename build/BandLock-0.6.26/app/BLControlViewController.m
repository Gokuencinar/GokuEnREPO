...
- (void)buildUI {
    ...
    version.text = @"BandLock Global 0.6.26 · diagnostic IPC build";
...
- (void)runDiagnosticSelfTest {
    BLBreadcrumbReset();
    BLBreadcrumb("RATTEST begin");
    __weak BLTelephonyManager *manager = self.manager;
    __weak typeof(self) weakSelf = self;
    [manager refreshWithCompletion:^(BOOL refreshOK, NSString *message) {
        BLBreadcrumbf("RATTEST initial refresh success=%d mode=%s",
                      refreshOK ? 1 : 0,
                      manager.networkMode.UTF8String ?: "(null)");
        if (!refreshOK) {
            BLBreadcrumb("RATTEST abort refresh failed");
            return;
        }

        [manager setNetworkModeLTEOnly:^(BOOL lteOK, NSString *lteMessage) {
            BOOL lteMode = [manager.networkMode rangeOfString:@"LTE" options:NSCaseInsensitiveSearch].location != NSNotFound;
            BLBreadcrumbf("RATTEST lte-only success=%d verified=%d mode=%s message=%s",
                          lteOK ? 1 : 0,
                          lteMode ? 1 : 0,
                          manager.networkMode.UTF8String ?: "(null)",
                          lteMessage.UTF8String ?: "(null)");

            [manager setNetworkModeAutomatic:^(BOOL automaticOK, NSString *automaticMessage) {
                NSString *mode = manager.networkMode ?: @"";
                BOOL automaticMode = [mode rangeOfString:@"Autom" options:NSCaseInsensitiveSearch].location != NSNotFound;
                BLBreadcrumbf("RATTEST automatic success=%d verified=%d mode=%s message=%s",
                              automaticOK ? 1 : 0,
                              automaticMode ? 1 : 0,
                              mode.UTF8String ?: "(null)",
                              automaticMessage.UTF8String ?: "(null)");

                [manager refreshWithCompletion:^(BOOL verifyOK, NSString *verifyMessage) {
                    NSString *verifiedMode = manager.networkMode ?: @"";
                    BOOL verifiedAutomatic = [verifiedMode rangeOfString:@"Autom" options:NSCaseInsensitiveSearch].location != NSNotFound;
                    BLBreadcrumbf("RATTEST final refresh success=%d automatic=%d mode=%s",
                                  verifyOK ? 1 : 0,
                                  verifiedAutomatic ? 1 : 0,
                                  verifiedMode.UTF8String ?: "(null)");

                    [manager openFieldTestWithCompletion:^(BOOL fieldOK, NSString *fieldMessage) {
                        BLBreadcrumbf("RATTEST field-test success=%d message=%s",
                                      fieldOK ? 1 : 0,
                                      fieldMessage.UTF8String ?: "(null)");
                        [weakSelf refreshDisplay];
                        BLBreadcrumb("RATTEST end");
                    }];
                }];
            }];
        }];
