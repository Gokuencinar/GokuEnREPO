...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.textLabel.text = ready ? BLT(@"Preparar bandas compatibles", @"Prepare compatible bands") : BLT(@"Actualizar módem y comprobar", @"Refresh modem and check");
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!self.manager.supportedBands.count) {
        ...
        [self.manager refreshWithCompletion:^(BOOL success, NSString *message) {
            ...
            if (success) [weakSelf prepareCompatibleBands];
            else [weakSelf showAlert:message];
        }];
    } else {
