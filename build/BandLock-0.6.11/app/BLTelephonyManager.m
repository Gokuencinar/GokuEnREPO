...
- (instancetype)initPrivate {
    ...
    if (self) {
        ...
        _hasReadState = NO;
        _busy = NO;
        BLProbeLog("manager init complete\n");
    }
    return self;
}
...
- (NSDictionary *)sendRequestSynchronously:(NSDictionary *)request {
    ...
    BLProbeLogf("ipc socket path length=%lu\n", (unsigned long)pathString.length);
    ...
    for (NSInteger attempt = 0; attempt < 5; attempt++) {
        BLProbeLogf("ipc connect attempt=%ld\n", (long)attempt);
        connected = connect(fd, (struct sockaddr *)&address, sizeof(address));
        BLProbeLogf("ipc connect result=%d errno=%d\n", connected, errno);
    ...
    while (remaining > 0) {
        BLProbeLogf("ipc write remaining=%lu\n", (unsigned long)remaining);
        ssize_t written = write(fd, bytes, remaining);
        BLProbeLogf("ipc write result=%ld errno=%d\n", (long)written, errno);
        if (written <= 0) {
            int savedErrno = errno;
            close(fd);
    ...
    BLProbeLog("ipc read loop begin\n");
    uint8_t buffer[4096];
    while (responseData.length < 131072) {
        ssize_t count = read(fd, buffer, sizeof(buffer));
        BLProbeLogf("ipc read result=%ld errno=%d\n", (long)count, errno);
        if (count < 0) {
            int savedErrno = errno;
            close(fd);
    ...
    close(fd);
    BLProbeLogf("ipc socket closed response=%lu\n", (unsigned long)responseData.length);
...
- (void)sendRequest:(NSDictionary *)request completion:(void (^)(NSDictionary *result))completion {
    BLProbeLog("ipc async request begin\n");
    if (self.busy) {
    ...
    dispatch_async(self.daemonQueue, ^{
        BLProbeLog("ipc daemon queue entered\n");
        NSDictionary *result = [self sendRequestSynchronously:request];
        BLProbeLog("ipc daemon queue completed sync\n");
        dispatch_async(dispatch_get_main_queue(), ^{
            BLProbeLog("ipc main completion entered\n");
            self.busy = NO;
            if (completion) completion(result ?: @{@"success": @NO, @"message": @"No result"});
            BLProbeLog("ipc main completion finished\n");
        });
    });
}
