...
- (NSMutableArray *)specifiers {
    ...
    statusGroup.name = @"Estado";
    [statusGroup setProperty:@"La lectura y los cambios solo se ejecutan mediante botones. «Banda en uso» intenta mostrar la celda servidora real, separada de las bandas permitidas." forKey:@"footerText"];
    [items addObject:statusGroup];
    ...
    [items addObject:[PSSpecifier preferenceSpecifierNamed:@"RAT actual" target:self set:nil get:@selector(ratValue) detail:nil cell:PSTitleValueCell edit:nil]];
    [items addObject:[PSSpecifier preferenceSpecifierNamed:@"Banda en uso" target:self set:nil get:@selector(servingBandValue) detail:nil cell:PSTitleValueCell edit:nil]];
    ...
    [items addObject:[PSSpecifier preferenceSpecifierNamed:@"LTE soportadas" target:self set:nil get:@selector(supportedLTEValue) detail:nil cell:PSTitleValueCell edit:nil]];
    [items addObject:[PSSpecifier preferenceSpecifierNamed:@"LTE permitidas" target:self set:nil get:@selector(activeLTEValue) detail:nil cell:PSTitleValueCell edit:nil]];

    PSSpecifier *toolsGroup = [PSSpecifier emptyGroupSpecifier];
    toolsGroup.name = @"Herramientas";
    [toolsGroup setProperty:@"Field Test Mode se intenta abrir directamente mediante la aplicación interna de Apple, sin marcar el código telefónico." forKey:@"footerText"];
    [items addObject:toolsGroup];

    PSSpecifier *fieldTest = [PSSpecifier preferenceSpecifierNamed:@"Abrir Field Test Mode"
        target:self set:nil get:nil detail:nil cell:PSButtonCell edit:nil];
    ...
    quickGroup.name = @"Selección rápida";
    [quickGroup setProperty:@"Orange España prepara B1, B3, B7, B8, B20 y B28 cuando el iPhone las soporte. Es un preset práctico; la disponibilidad exacta depende del despliegue de tu zona." forKey:@"footerText"];
    [items addObject:quickGroup];

    PSSpecifier *orange = [PSSpecifier preferenceSpecifierNamed:@"Preset Orange España"
        target:self set:nil get:nil detail:nil cell:PSButtonCell edit:nil];
    orange.buttonAction = @selector(selectOrangeSpain);
    [orange setProperty:@(_blHasRead) forKey:@"enabled"];
    [items addObject:orange];

    PSSpecifier *all = [PSSpecifier preferenceSpecifierNamed:@"Seleccionar todas"
        target:self set:nil get:nil detail:nil cell:PSButtonCell edit:nil];
    all.buttonAction = @selector(selectAllBands);
    [all setProperty:@(_blHasRead) forKey:@"enabled"];
    [items addObject:all];

    PSSpecifier *none = [PSSpecifier preferenceSpecifierNamed:@"Deseleccionar todas"
        target:self set:nil get:nil detail:nil cell:PSButtonCell edit:nil];
    none.buttonAction = @selector(deselectAllBands);
    [none setProperty:@(_blHasRead) forKey:@"enabled"];
    [items addObject:none];

    PSSpecifier *selectionGroup = [PSSpecifier emptyGroupSpecifier];
    selectionGroup.name = @"Bandas LTE";
    [selectionGroup setProperty:(_blHasRead
        ? @"Cada interruptor muestra la banda y su frecuencia nominal. Los cambios no llegan al módem hasta pulsar «Aplicar selección LTE»."
        : @"Pulsa primero «Leer bandas».")
        forKey:@"footerText"];
    [items addObject:selectionGroup];

    if (_blSupportedLTE.count) {
        for (NSNumber *band in _blSupportedLTE) {
            PSSpecifier *toggle = [PSSpecifier preferenceSpecifierNamed:BLBandTitle(band)
                target:self
        ...
        PSSpecifier *empty = [PSSpecifier preferenceSpecifierNamed:@"Sin datos LTE todavía"
    ...
    actionsGroup.name = @"Aplicar y restaurar";
    [actionsGroup setProperty:@"Antes de aplicar se guarda la selección LTE anterior. Nunca se permite escribir una selección vacía." forKey:@"footerText"];
    ...
    PSSpecifier *restorePrevious = [PSSpecifier preferenceSpecifierNamed:@"Restaurar selección anterior"
        target:self set:nil get:nil detail:nil cell:PSButtonCell edit:nil];
    restorePrevious.buttonAction = @selector(restorePreviousLTE);
    [restorePrevious setProperty:@(_blHasRead && _blPreviousLTE.count > 0) forKey:@"enabled"];
    [items addObject:restorePrevious];

    PSSpecifier *restoreAll = [PSSpecifier preferenceSpecifierNamed:@"Restaurar todas las LTE"
        target:self set:nil get:nil detail:nil cell:PSButtonCell edit:nil];
    restoreAll.buttonAction = @selector(restoreAllLTE);
    [restoreAll setProperty:@(_blHasRead) forKey:@"enabled"];
    [items addObject:restoreAll];

    PSSpecifier *diagnosticGroup = [PSSpecifier emptyGroupSpecifier];
    diagnosticGroup.name = @"Diagnóstico";
    [diagnosticGroup setProperty:@"Ruta SSH: /rootfs/private/var/mobile/Library/Logs/BandLock/BandLock-last.txt. Los registros no incluyen el contexto completo de la SIM." forKey:@"footerText"];
...
- (NSString *)versionValue { return @"0.3.8"; }
...
- (NSString *)modeValue {
    ...
    return [supported isEqualToSet:active] ? @"Automático / todas" : BLBandList(_blActiveLTE);
}
...
- (void)savePreviousLTE:(NSArray<NSNumber *> *)bands {
    ...
    NSDictionary *state = @{@"previousLTE": clean};
    if ([state writeToFile:BLStatePath atomically:YES]) _blPreviousLTE = clean;
}
...
- (void)selectOrangeSpain {
    ...
    NSSet *supported = [NSSet setWithArray:_blSupportedLTE];
    NSArray *orange = @[@1, @3, @7, @8, @20, @28];
    NSMutableArray *available = [NSMutableArray array];
    for (NSNumber *band in orange) if ([supported containsObject:band]) [available addObject:band];
    _blSelectedLTE = [NSMutableSet setWithArray:available];
    _blStatus = @"Preset preparado";
    _blDetail = [NSString stringWithFormat:@"Orange España: %@. Pulsa «Aplicar selección LTE» para escribirlo.", BLBandList(available)];
    [self rebuildUI];
}
...
- (void)restoreAllLTE {
    ...
    [self applyLTEBands:_blSupportedLTE action:@"Restaurar todas" savePrevious:YES];
}
...
- (void)openFieldTestMode {
    ...
    @try {
        ...
        if (workspaceClass && [workspaceClass respondsToSelector:defaultSelector]) {
            ...
            if (workspace && [workspace respondsToSelector:openSelector]) {
                launched = BLMsgBoolObj(workspace, openSelector, @"com.apple.fieldtest");
            }
        }
    }
    ...
    if (!launched) {
        UIAlertController *alert = [UIAlertController
            alertControllerWithTitle:@"Field Test Mode"
            message:@"No se pudo abrir directamente com.apple.fieldtest en este sistema. Puedes abrirlo desde Teléfono marcando *3001#12345#*."
