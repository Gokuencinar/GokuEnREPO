...
- (void)manageCountryBandsTapped:(UIButton *)sender {
    ...
    NSArray *compatible = BLIntersectBands(self.manager.supportedBands, countryBands);
    if (!compatible.count) {
        [self showAlert:BLT(@"Este iPhone no reporta bandas LTE compatibles con el país seleccionado.", @"This iPhone does not report LTE bands compatible with the selected country.")];
        return;
    }
