...
- (instancetype)initPrivate {
    ...
    if (self) {
        ...
        _supportedBands = BLSortedBands(state[@"supportedLTE"]);
        _activeBands = BLSortedBands(state[@"activeLTE"]);
        _pendingBands = BLSortedBands(state[@"pendingLTE"]);
...
- (void)consumeSnapshotOnMain:(NSDictionary *)snapshot resetPending:(BOOL)resetPending {
    ...
    BLWriteState(@{@"supportedLTE": self.supportedBands ?: @[], @"activeLTE": self.activeBands ?: @[], @"pendingLTE": self.pendingBands ?: @[]});
}
...
- (void)setPendingBands:(NSArray<NSNumber *> *)bands {
    ...
    self.pendingBands = clean;
    BLWriteState(@{@"pendingLTE": clean});
    [NSNotificationCenter.defaultCenter postNotificationName:BLPendingSelectionDidChangeNotification object:self];
}
...
- (BOOL)writeLTEBands:(NSArray<NSNumber *> *)bands usingQuery:(NSDictionary *)query errorText:(NSString **)errorText {
    ...
    if (modifiedBandInfo && [modifiedBandInfo respondsToSelector:setActiveDictionary]) BLMsgSetObj(modifiedBandInfo, setActiveDictionary, newActive);
    else {
        ...
        if (allocated && [allocated respondsToSelector:initSelector]) modifiedBandInfo = BLMsg2(allocated, initSelector, currentSupported, newActive);
    }
...
- (void)writeLogAction:(NSString *)action detail:(NSString *)detail {
    ...
    NSString *text = [NSString stringWithFormat:@"BandLock-Version: 0.6.0\nTimestamp: %@\nAction: %@\nRAT: %@\nServing-Band: %@\nSupported-LTE: %@\nActive-LTE: %@\nPending-LTE: %@\nDetail: %@\n",
...
- (BOOL)openFieldTestWithError:(NSString **)errorText {
    ...
    @try {
        ...
        if (workspaceClass && [workspaceClass respondsToSelector:defaultSelector]) {
            ...
            if (workspace && [workspace respondsToSelector:openAppSelector] && BLMsgBoolObj(workspace, openAppSelector, bundleID)) return YES;
        }
    } @catch (NSException *exception) {
        if (errorText) *errorText = exception.reason ?: exception.name;
        return NO;
    }
    if (errorText) *errorText = BLT(@"No se pudo abrir FTMInternal.", @"Could not open FTMInternal.");
    return NO;
}
