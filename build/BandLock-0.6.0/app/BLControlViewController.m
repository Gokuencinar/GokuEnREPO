...
- (void)viewDidLoad {
    ...
    [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(pendingChanged:) name:BLPendingSelectionDidChangeNotification object:nil];
}
...
- (void)pendingChanged:(NSNotification *)note { [self.tableView reloadData]; }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    return [self valueCellWithTitle:BLT(@"Versión", @"Version") value:@"0.6.0 Global App"];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == BLControlSectionBands) {
        if (indexPath.row == 0) {
            if (!self.manager.supportedBands.count) { [self refreshStatus]; return; }
    ...
    if (indexPath.section == BLControlSectionTools && indexPath.row == 0) {
        NSString *error = nil;
        if (![self.manager openFieldTestWithError:&error]) [self showResult:NO message:error];
    }
}
