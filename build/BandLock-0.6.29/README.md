# BandLock 0.6.29 Global App

BandLock 0.6.29 moves the public tweak out of the iOS Settings app and into a dedicated UIKit application installed on the Home Screen.

### 0.6.29 Field Test dialer diagnostic

- Diagnostic candidate focused on opening Apple's Field Test menu on iOS 16.3.
- Field Test is handed to the real Phone dialer with `tel://*3001%2312345%23*`, so MobilePhone itself can interpret Apple's special code instead of CoreTelephony treating it as an ordinary call.
...
BandLock 0.6.29 continues to target LTE/4G band control only; it does not claim 5G NR band locking.
