...
- (NSDictionary *)snapshotFromQuery:(NSDictionary *)query {
    ...
    if ([selection rangeOfString:@"Automatic" options:NSCaseInsensitiveSearch].location != NSNotFound) mode = BLT(@"Automático", @"Automatic");
    else if ([selection rangeOfString:@"LTE" options:NSCaseInsensitiveSearch].location != NSNotFound) mode = BLT(@"Solo LTE / 4G", @"LTE / 4G only");
    else if (![selection isEqualToString:@"—"]) mode = selection;
...
- (void)writeLogAction:(NSString *)action detail:(NSString *)detail {
    ...
    NSString *text = [NSString stringWithFormat:@"BandLock-Version: 0.6.3\nTimestamp: %@\nAction: %@\nRAT: %@\nServing-Band: %@\nSupported-LTE: %@\nActive-LTE: %@\nPending-LTE: %@\nDetail: %@\n",
...
- (BOOL)openFieldTestWithError:(NSString **)errorText {
    ...
    @try {
        void *coreTelephony = dlopen("/System/Library/Frameworks/CoreTelephony.framework/CoreTelephony", RTLD_NOW | RTLD_LOCAL);
        if (coreTelephony) {
            id (*CTCallDialFn)(NSString *) = (id (*)(NSString *))dlsym(coreTelephony, "CTCallDial");
            if (CTCallDialFn) {
                id call = CTCallDialFn(code);
                if (call) return YES;
            }
        }

        void *telephonyUtilities = dlopen("/System/Library/PrivateFrameworks/TelephonyUtilities.framework/TelephonyUtilities", RTLD_NOW | RTLD_LOCAL);
        if (telephonyUtilities) {
            Class callCenterClass = NSClassFromString(@"TUCallCenter");
            SEL sharedSelector = NSSelectorFromString(@"sharedInstance");
            SEL dialSelector = NSSelectorFromString(@"dial:service:");
            if (callCenterClass && [callCenterClass respondsToSelector:sharedSelector]) {
                id callCenter = BLMsg0((id)callCenterClass, sharedSelector);
                if (callCenter && [callCenter respondsToSelector:dialSelector]) {
                    id call = ((id (*)(id, SEL, id, int))objc_msgSend)(callCenter, dialSelector, code, 0);
                    if (call) return YES;
                }
            }
        }
        ...
        if (workspaceClass && [workspaceClass respondsToSelector:defaultSelector]) {
            ...
            if (workspace && [workspace respondsToSelector:openAppSelector]) (void)BLMsgBoolObj(workspace, openAppSelector, bundleID);
        }
    } @catch (NSException *exception) {
        if (errorText) *errorText = exception.reason ?: exception.name;
        return NO;
    }
    if (errorText) *errorText = BLT(@"No se pudo invocar el código de Field Test en este iOS.", @"Could not invoke the Field Test service code on this iOS version.");
    return NO;
}
