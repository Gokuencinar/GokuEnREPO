...
- (void)openFieldTestWithCompletion:(BLActionCompletion)completion {
    dispatch_async(dispatch_get_main_queue(), ^{
        NSURL *url = [NSURL URLWithString:@"tel://*3001%2312345%23*"];
        UIApplication *application = UIApplication.sharedApplication;
        if (!url || ![application canOpenURL:url]) {
            NSString *message = BLT(@"iOS no acepta la URL de Field Test.", @"iOS rejected the Field Test dialer URL.");
            self.statusText = BLT(@"Field Test no disponible", @"Field Test unavailable");
            self.detailText = message;
        ...
        [application openURL:url options:@{} completionHandler:^(BOOL success) {
            dispatch_async(dispatch_get_main_queue(), ^{
                NSString *message = success
                    ? BLT(@"Código de Field Test enviado a Teléfono.", @"Field Test code handed to Phone.")
                    : BLT(@"Teléfono no aceptó el código de Field Test.", @"Phone did not accept the Field Test code.");
                self.statusText = success ? BLT(@"Abriendo Field Test", @"Opening Field Test") : BLT(@"Field Test no disponible", @"Field Test unavailable");
                self.detailText = message;
