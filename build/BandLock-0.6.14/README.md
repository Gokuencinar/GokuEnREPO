# BandLock 0.6.14 Global App

BandLock 0.6.14 moves the public tweak out of the iOS Settings app and into a dedicated UIKit application installed on the Home Screen.

### 0.6.14 IPC correction

- The UI app no longer calls `jbroot()` or links `libroothide` just to locate the daemon socket.
- App and daemon use the short jbroot-native UNIX socket path `/tmp/bandlockd.sock`.
- RootHide performs its own `bind()`/`connect()` path translation exactly once; BandLock no longer feeds an already-expanded `.jbroot-*` path back into that layer.
- The shorter socket name also keeps the translated path comfortably below Darwin's `sockaddr_un.sun_path` limit.
- Package scripts remove the previous `/tmp/com.gokuencinar.bandlockd.sock` socket during migration.
...
BandLock 0.6.14 continues to target LTE/4G band control only; it does not claim 5G NR band locking.
