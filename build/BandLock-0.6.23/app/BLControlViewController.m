...
- (void)viewDidLoad {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        BLBreadcrumb("diagnostic selftest fired");
        [self runDiagnosticSelfTest];
    });
}
...
- (void)buildUI {
    ...
    version.text = @"BandLock Global 0.6.23 · diagnostic IPC build";
...
- (void)runDiagnosticSelfTest {
    BLBreadcrumbReset();
    BLBreadcrumb("RESTORETEST begin");
    __weak BLTelephonyManager *manager = self.manager;
    __weak typeof(self) weakSelf = self;
    NSArray<NSNumber *> *targetBands = @[@1, @3, @7, @8, @20, @28, @38];

    [manager refreshWithCompletion:^(BOOL refreshOK, NSString *message) {
        BLBreadcrumbf("RESTORETEST initial refresh success=%d active-count=%lu",
                      refreshOK ? 1 : 0,
                      (unsigned long)manager.activeBands.count);
        if (!refreshOK) {
            BLBreadcrumb("RESTORETEST abort refresh failed");
            return;
        }

        [manager setPendingBands:targetBands];
        BLBreadcrumb("RESTORETEST target prepared");
        [manager applyPendingBandsWithCompletion:^(BOOL initialRestoreOK, NSString *initialRestoreMessage) {
            BLBreadcrumbf("RESTORETEST initial restore success=%d message=%s",
                          initialRestoreOK ? 1 : 0,
                          initialRestoreMessage.UTF8String ?: "(null)");

            [manager refreshWithCompletion:^(BOOL verifyRestoreOK, NSString *verifyRestoreMessage) {
                NSSet *target = [NSSet setWithArray:targetBands];
                NSSet *restoredSet = [NSSet setWithArray:manager.activeBands ?: @[]];
                BOOL restored = verifyRestoreOK && [target isEqualToSet:restoredSet];
                BLBreadcrumbf("RESTORETEST initial verify success=%d restored=%d active-count=%lu",
                              verifyRestoreOK ? 1 : 0,
                              restored ? 1 : 0,
                              (unsigned long)manager.activeBands.count);
                if (!restored) {
                    [weakSelf refreshDisplay];
                    BLBreadcrumb("RESTORETEST abort target not restored");
                    return;
                }

                [manager restoreAllSupportedBandsWithCompletion:^(BOOL restoreAllOK, NSString *restoreAllMessage) {
                    BLBreadcrumbf("RESTORETEST restore-all success=%d message=%s",
                                  restoreAllOK ? 1 : 0,
                                  restoreAllMessage.UTF8String ?: "(null)");

                    [manager restorePreviousBandsWithCompletion:^(BOOL restorePreviousOK, NSString *restorePreviousMessage) {
                        BLBreadcrumbf("RESTORETEST restore-previous success=%d message=%s",
                                      restorePreviousOK ? 1 : 0,
                                      restorePreviousMessage.UTF8String ?: "(null)");

                        // Always force the verified target once more as a safety net.
                        [manager setPendingBands:targetBands];
                        [manager applyPendingBandsWithCompletion:^(BOOL finalRestoreOK, NSString *finalRestoreMessage) {
                            BLBreadcrumbf("RESTORETEST final restore success=%d message=%s",
                                          finalRestoreOK ? 1 : 0,
                                          finalRestoreMessage.UTF8String ?: "(null)");

                            [manager setNetworkModeAutomatic:^(BOOL automaticOK, NSString *automaticMessage) {
                                BLBreadcrumbf("RESTORETEST automatic success=%d message=%s",
                                              automaticOK ? 1 : 0,
                                              automaticMessage.UTF8String ?: "(null)");

                                [manager refreshWithCompletion:^(BOOL finalVerifyOK, NSString *finalVerifyMessage) {
                                    NSSet *finalSet = [NSSet setWithArray:manager.activeBands ?: @[]];
                                    BOOL finalRestored = finalVerifyOK && [target isEqualToSet:finalSet];
                                    BLBreadcrumbf("RESTORETEST final verify success=%d restored=%d active-count=%lu",
                                                  finalVerifyOK ? 1 : 0,
                                                  finalRestored ? 1 : 0,
                                                  (unsigned long)manager.activeBands.count);
                                    [weakSelf refreshDisplay];
                                    BLBreadcrumb("RESTORETEST end");
                                }];
                            }];
                        }];
                    }];
