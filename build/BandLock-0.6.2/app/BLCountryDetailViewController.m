...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.textLabel.text = ready ? BLT(@"Preparar bandas compatibles", @"Prepare compatible bands") : BLT(@"Actualizar desde Control primero", @"Refresh from Control first");
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!self.manager.supportedBands.count) {
        [self showAlert:BLT(@"Pulsa «Actualizar estado» en Control antes de preparar un país. Esta pantalla ya no ejecuta CoreTelephony implícitamente.", @"Tap “Refresh status” in Control before preparing a country. This screen no longer invokes CoreTelephony implicitly.")];
    } else {
