...
- (NSMutableArray *)specifiers {
    ...
    [networkGroup setProperty:@"Automático permite el comportamiento normal de iOS. «Solo LTE / 4G» impide el fallback a 3G/EDGE mientras esté activo. Si VoLTE no está disponible, las llamadas pueden verse afectadas." forKey:@"footerText"];
    [items addObject:networkGroup];

    Class listItemsController = NSClassFromString(@"PSListItemsController");
    PSSpecifier *networkMode = [PSSpecifier preferenceSpecifierNamed:@"Tecnología preferida"
        target:self
...
- (NSString *)versionValue { return @"0.4.2"; }
- (NSString *)networkModeStatusValue {
    return _blConfiguredRATMode ?: @"Sin consultar";
}

- (id)networkModeValue:(__unused PSSpecifier *)specifier {
    if ([_blRatSelectionRaw rangeOfString:@"LTE" options:NSCaseInsensitiveSearch].location != NSNotFound &&
        [_blRatSelectionRaw rangeOfString:@"Automatic" options:NSCaseInsensitiveSearch].location == NSNotFound) {
        return @"lte";
    }
    NSString *saved = BLReadState()[@"networkModeRequested"];
    return [saved isEqualToString:@"lte"] ? @"lte" : @"automatic";
}

- (void)setNetworkModeValue:(id)value specifier:(__unused PSSpecifier *)specifier {
    NSString *mode = [value isKindOfClass:[NSString class]] ? value : @"automatic";
    if ([mode isEqualToString:@"lte"]) {
        UIAlertController *alert = [UIAlertController
            alertControllerWithTitle:@"Solo LTE / 4G"
            message:@"Mientras este modo esté activo el módem no debería hacer fallback a 3G o EDGE. Si no hay LTE disponible, puedes quedarte sin servicio temporalmente."
            preferredStyle:UIAlertControllerStyleAlert];
        __weak typeof(self) weakSelf = self;
        [alert addAction:[UIAlertAction actionWithTitle:@"Cancelar" style:UIAlertActionStyleCancel handler:^(__unused UIAlertAction *action) {
            [weakSelf rebuildUI];
        }]];
        [alert addAction:[UIAlertAction actionWithTitle:@"Activar" style:UIAlertActionStyleDestructive handler:^(__unused UIAlertAction *action) {
            [weakSelf applyNetworkModeSelection:BLRATLTE preferred:BLRATLTE label:@"Solo LTE / 4G" stateValue:@"lte"];
        }]];
        [self presentViewController:alert animated:YES completion:nil];
        return;
    }

    [self applyNetworkModeSelection:BLRATAutomatic preferred:BLRATAutomatic label:@"Automático" stateValue:@"automatic"];
}
