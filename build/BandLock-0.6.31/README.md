# BandLock 0.6.31 Global App

BandLock 0.6.31 moves the public tweak out of the iOS Settings app and into a dedicated UIKit application installed on the Home Screen.

### 0.6.31 MobilePhone special-code bridge diagnostic

- Diagnostic candidate focused on opening Apple's Field Test menu on iOS 16.3.
- URLs and CoreTelephony dialing treat `*3001#12345#*` as an ordinary phone number on the tested iPhone XS, while entering it manually in Phone works.
- A tiny tweak is filtered only to `com.apple.mobilephone`. BandLock opens Phone, sends a Darwin notification, and the bridge invokes MobilePhone/TelephonyUI's own `launchFieldTestIfNeeded:` special-code branch before any normal dial request is built.
...
BandLock 0.6.31 continues to target LTE/4G band control only; it does not claim 5G NR band locking.
