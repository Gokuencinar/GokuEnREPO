...
- (void)viewDidLoad {
    ...
    @try {
        ...
        if ([table isKindOfClass:[UITableView class]] && !table.tableHeaderView) {
            ...
            subtitle.text = @"Orange España · LTE";
            ...
            caption.text = @"Selección manual · sin procesos en segundo plano";
...
- (NSMutableArray *)specifiers {
    ...
    connectionGroup.name = @"Conexión";
    [connectionGroup setProperty:@"«Banda conectada» intenta leer la celda servidora real. «LTE permitidas» indica qué bandas puede usar el módem tras aplicar un bloqueo." forKey:@"footerText"];
    ...
    PSSpecifier *profilesGroup = [PSSpecifier emptyGroupSpecifier];
    profilesGroup.name = @"Perfiles rápidos";
    [profilesGroup setProperty:@"Solo se muestran las bandas LTE principales para España: 700, 800, 900, 1800, 2100 y 2600 MHz. La disponibilidad real depende de la red Orange de tu zona." forKey:@"footerText"];
    [items addObject:profilesGroup];

    NSArray *profileRows = @[
        @[@"Orange España · todas", NSStringFromSelector(@selector(selectOrangeSpain))],
        @[@"Cobertura · 700/800/900", NSStringFromSelector(@selector(selectCoverageBands))],
        @[@"Capacidad · 1800/2100/2600", NSStringFromSelector(@selector(selectCapacityBands))],
        @[@"B3 + B7 · 1800/2600", NSStringFromSelector(@selector(selectB3B7))]
    ];
    for (NSArray *row in profileRows) {
        PSSpecifier *button = [PSSpecifier preferenceSpecifierNamed:row[0]
            target:self set:nil get:nil detail:nil cell:PSButtonCell edit:nil];
        button.buttonAction = NSSelectorFromString(row[1]);
        [button setProperty:@(_blHasRead) forKey:@"enabled"];
        [items addObject:button];
    }

    PSSpecifier *coverageGroup = [PSSpecifier emptyGroupSpecifier];
    coverageGroup.name = @"Cobertura";
    [coverageGroup setProperty:@"Frecuencias bajas: mejor propagación y penetración en interiores. Que una banda aparezca aquí no significa que Orange la tenga activa en la celda donde estás." forKey:@"footerText"];
    [items addObject:coverageGroup];

    for (NSNumber *band in @[@28, @20, @8]) {
        if (![_blSupportedLTE containsObject:band]) continue;
        PSSpecifier *toggle = [PSSpecifier preferenceSpecifierNamed:[NSString stringWithFormat:@"B%@ · %@", band, BLBandUseText(band)]
            target:self set:@selector(setBandSwitchValue:specifier:) get:@selector(bandSwitchValue:)
    ...
    PSSpecifier *capacityGroup = [PSSpecifier emptyGroupSpecifier];
    capacityGroup.name = @"Capacidad y velocidad";
    [capacityGroup setProperty:@"Frecuencias medias/altas. En tu prueba B3 y B7 ya han sido aceptadas y verificadas por el módem." forKey:@"footerText"];
    [items addObject:capacityGroup];

    for (NSNumber *band in @[@3, @1, @7]) {
        if (![_blSupportedLTE containsObject:band]) continue;
        PSSpecifier *toggle = [PSSpecifier preferenceSpecifierNamed:[NSString stringWithFormat:@"B%@ · %@", band, BLBandUseText(band)]
            target:self set:@selector(setBandSwitchValue:specifier:) get:@selector(bandSwitchValue:)
            detail:nil cell:PSSwitchCell edit:nil];
    ...
    if (!_blHasRead) {
        PSSpecifier *hintGroup = [PSSpecifier emptyGroupSpecifier];
        hintGroup.name = @"Bandas";
        [hintGroup setProperty:@"Pulsa «Actualizar estado» para cargar las bandas compatibles con este iPhone." forKey:@"footerText"];
        [items addObject:hintGroup];
    }

    PSSpecifier *applyGroup = [PSSpecifier emptyGroupSpecifier];
    applyGroup.name = @"Aplicar cambios";
    [applyGroup setProperty:@"Antes de aplicar se guarda la configuración LTE anterior. BandLock vuelve a leer el módem después de escribir y solo muestra «Aplicado y verificado» si coincide." forKey:@"footerText"];
    [items addObject:applyGroup];
    ...
    [toolsGroup setProperty:@"Abre directamente FTMInternal-4 (com.apple.FTMInternal), la aplicación interna usada por iOS para Field Test Mode." forKey:@"footerText"];
...
- (NSString *)versionValue { return @"0.3.9"; }
